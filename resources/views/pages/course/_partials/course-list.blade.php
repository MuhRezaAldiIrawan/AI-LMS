    ...
    <div class="mentor-card rounded-8 card border border-gray-100">
        <div class="card-body p-8">
            <a href="course-details.html" class="bg-main-100 rounded-8 overflow-hidden text-center mb-8 h-164 flex-center p-8">
                <img src="{{ asset('storage' . '/' . $course->thumbnail) }}" alt="Course Image" class="w-100 h-100 object-fit-cover" style="border-radius: 8px">
            </a>
            <div class="p-8">
                <span
                    class="text-13 py-2 px-10 rounded-pill bg-success-50 text-success-600 mb-16">{{ $course->category->name }}</span>
                <h5 class="mb-0"><a href="course-details.html" class="hover-text-main-600">{{ $course->title }}</a>
                </h5>
                ...
                <div class="text-sm text-gray-600 dark:text-gray-400  flex flex-wrap items-center gap-x-6 gap-y-3" style="display: flex; justify-content: space-between; margin-top: 15px">
                    <!-- Lesson -->
                    ...
                    <span class="flex items-center gap-2" title="Peserta">
                        <svg class="w-15 h-15 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="{{ route('course.show', $course->id) }}" class="btn btn-outline-main rounded-pill py-9 w-100 mt-24">Kelola Kursus</a>
            </div>
        </div>
    </div>
</div>
