...
@section('content')
    <div class="breadcrumb-with-buttons mb-24 flex-between flex-wrap gap-8">
        ...
        <div class="breadcrumb mb-24">
            <ul class="flex-align gap-4">
                ...
                <li>
                    <span class="text-main-600 fw-normal text-15">Tipe Kursus</span>
                </li>
            </ul>
        </div>
        <!-- Breadcrumb End -->
    ...
    <div class="card overflow-hidden">
        <div class="card-body p-0 overflow-x-auto">
            <table id="courseTypeTable" class="table table-hover  align-middle">
                <thead>
                    <tr>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            $('#courseTypeTable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('coursetype.get-data') }}",
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex'
                    },
        ...
        $(document).on('click', '.btn-coursetype-delete', function(e) {
            e.preventDefault();
            let id = $(this).data('id');

            let url = "{{ route('coursetype.destroy', ':id') }}";
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(data) {
                            Swal.fire({
                                title: 'Terhapus!',
                                text: 'Data Tipe Kursus Telah berhasil dihapus.',
                                icon: 'success',
                                timer: 2000
                            });
                            $('#courseTypeTable').DataTable().ajax.reload();
                        }
                    })
                }
            })
